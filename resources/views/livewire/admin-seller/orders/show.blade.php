    ...
    <div class="space-y-4">
        <x-card>
            <header class="flex justify-between items-center">
                ...
                <div>
                    <x-icon-button @click="$dispatch('open-modal', 'detail-modal')" icon="eye" />
                    <x-modal name="detail-modal" title="Order Details">
            ...
            {{-- <div>
                @if ($admin)
                    <x-badge color="info-outline" value="{{ $order->sales->count() }}" />
                @else
                    <x-badge color="info-outline"
                        value="{{ $order->sales->where('product.item.seller.id', Auth::user()->seller->id)->count() }}" />
                @endif
            </div> --}}
